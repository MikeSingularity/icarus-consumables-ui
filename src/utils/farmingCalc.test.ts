...
const makeItem = (
  ...
  tier: '1',
  base_stats: {},
  modifiers: Object.fromEntries(modifiers.map((m) => [m, 1800])),
  modifier_stats: {},
  recipes,
  tags: [],
...
const makeRecipe = (
  id: string,
  inputs: Array<{ id: string; count: number; is_generic?: boolean }>,
  outputId: string,
  yieldsCount = 1
): Recipe => ({
  id,
  inputs: inputs.map((i) => ({
    id: i.id,
    count: i.count,
    display_name: i.id.replace(/_/g, ' '),
    is_generic: i.is_generic ?? false,
  })),
  outputs: [{ id: outputId, count: yieldsCount, display_name: outputId }],
...
const recipeFlour = makeRecipe('recipe_flour', [{ id: 'Wheat', count: 2 }], 'Flour');
...
const recipeBread = makeRecipe('recipe_bread', [{ id: 'Flour', count: 3 }], 'Bread');
...
stew.modifiers['stew_mod'] = 0;
const recipeStew = makeRecipe('recipe_stew', [{ id: 'Raw_Meat', count: 2 }], 'Stew');
...
const defaultParams = (
  loadoutItems: Item[],
  overrides: Partial<
    Pick<FarmingParams, 'servingsOverrides' | 'recipeOverrides' | 'genericSelections'>
  > = {}
): FarmingParams => ({
  loadoutItems,
...
describe('getRecipeYieldCount', () => {
  ...
  it('returns yields_count for multiple outputs, matching by id', () => {
    const multiRecipe: Recipe = {
      id: 'r_multi',
      inputs: [],
      outputs: [
        { id: 'By_Product', count: 1, display_name: 'By Product' },
        { id: 'Cake_Slice', count: 4, display_name: 'Slices' },
      ],
      benches: ['Bench'],
      requirements: {},
    };
...
describe('computeFarmingResult', () => {
  it('resolves leaf farmable ingredient into crop plot entry', () => {
    ...
    expect(result.cropPlots[0].id).toBe('Wheat');
  ...
  it('resolves non-farmable ingredient into stockpile entry', () => {
    ...
    expect(result.stockpile[0].id).toBe('Raw_Meat');
  ...
  it('sums ingredients across multiple loadout items sharing the same leaf', () => {
    ...
    const recipeFlatBread = makeRecipe(
      'recipe_flat_bread',
      [{ id: 'Flour', count: 2 }],
      'Flat_Bread'
    );
  ...
  it('calculates farming for loadout items with growth_data but no recipe', () => {
  ...
  it('resolves generic ingredient to first option by default', () => {
    ...
    const genericRecipe = makeRecipe(
      'recipe_generic',
      [{ id: 'Any_Vegetable', count: 2, is_generic: true }],
      'Generic_Food'
    );
    ...
    expect(result.cropPlots).toHaveLength(1);
    expect(result.cropPlots[0].id).toBe('Carrot');
  ...
  it('stops recursion for generic items marked as is_leaf', () => {
    ...
    const fishRecipe = makeRecipe(
      'recipe_fish',
      [{ id: 'Any_Raw_Fish', count: 1, is_generic: true }],
      'Steamed_Fish'
    );
    ...
    const recipeButcher = makeRecipe('recipe_butcher', [{ id: 'carcass', count: 1 }], 'fish01');
    ...
    expect(result.stockpile).toHaveLength(1);
    expect(result.stockpile[0].id).toBe('Any_Raw_Fish');
  ...
  it('handles source_item pieces with fallback to parent recipe (piece has no own recipe)', () => {
    ...
    const recipeWholeCake: Recipe = {
      ...
      inputs: [{ id: 'Flour', count: 3, display_name: 'Flour', is_generic: false }],
      outputs: [{ id: 'Cake_Slice', count: 4, display_name: 'Slices' }],
      benches: ['Bench'],
      requirements: {},
    };
    const params: FarmingParams = {
  ...
  it('handles source_item pieces with own recipe (real-world chocolate cake pattern)', () => {
    ...
    const recipeChocolateCake: Recipe = {
      ...
      outputs: [
        {
          id: 'chocolatecakepiece',
          count: 8,
          display_name: 'Chocolate Cake Piece',
          recipe_produces: { id: 'chocolatecake', count: 1 },
        },
      ],
